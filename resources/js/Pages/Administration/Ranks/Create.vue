...
function submit() { form.post(route('ranks'), { onSuccess: () => form.reset() }); }
</script>

<template>

    <Head :title="t('ranks.create')" />
    <AuthenticatedLayout>
        <template #header>
            <nav class="breadcrumb">
                ...
                <span class="bc-sep">/</span>
                <span class="bc-current">{{ t('ranks.create') }}</span>
            </nav>
        </template>

        <div class="create-page">
            <h1 class="page-title">{{ t('ranks.create') }}</h1>
